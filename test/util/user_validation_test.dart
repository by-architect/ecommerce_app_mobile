...
void main() {
  ...
  test("userValidation_validateUser_validUser", () {
    ...
    var userState = UserRequestState(name: user.name,surname: user.surname,email: user.email,password: user.password,phoneNo: ("(555) 435-4355"));
  ...
  test("userValidation_validateUser_invalidPhone", () {
    var user = FakeUserModels.testValidUser;
    var userState = UserRequestState(name: user.name,surname: user.surname,email: user.email,password: user.password,phoneNo: ("329857e29857982347"));
    var validation = UserValidation.validate(userState);
    Log.debug("user_validation:", validation.message);
    expect(validation.success, false);
  });
